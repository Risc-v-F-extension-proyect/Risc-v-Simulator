...
public:
  ...
  QString extensionDescription(const QString &ext) const override {
    if (ext == "F")
      return "Simple Flotant point instructions";
    if (ext == "M")
...
enum OpcodeID {
  ...
  AUIPC = 0b0010111,
  INVALID = 0b0,
  FP       = 0b1010011, //general F extentio ofcode
  FP_MADD  = 0b1000011, //FMADD.S  ofcode
  FP_MSUB  = 0b1000111, //FMSUB.S ofcode
  FP_NMSUB = 0b1001011, //FNMSUB.S ofcode
  FP_NMADD = 0b1001111, //FNMADD.S ofcode
  FP_LW    = 0b0000111, //FLW.s ofcode
  FP_SW    = 0b0100111, //FSW.s ofcode
};
enum QuadrantID {
  QUADRANT0 = 0b00,
  QUADRANT1 = 0b01,
...
/// of the instruction
template <unsigned funct5_r4, unsigned N = 32>
struct OpPartFunct5_R4 : public OpPart<funct5_r4, BitRange<2, 6, N>> {};
/// All RISC-V Fmt opcode parts are defined as a 2-bit field in bits 25-26
/// of the instruction
...
////Petru modificaciones
/// The RISC-V Rs2 field contains a source register index.
/// It is defined as a 5-bit field in bits 27-21 of the instruction
template <unsigned tokenIndex>
struct RegRs3
    : public GPR_Reg<RegRs3<tokenIndex>, tokenIndex, BitRange<27, 31>> {
  constexpr static std::string_view getName() { return "rs3"; }
};
////Petru modificaciones
/// The RISC-V Rs2 field contains a source register index.
/// It is defined as a 3-bit field in bits 12-14 of the instruction
template <unsigned tokenIndex>
struct RegRm
    : public GPR_Reg<RegRm<tokenIndex>, tokenIndex, BitRange<12, 14>> {
  constexpr static std::string_view getName() { return "rm"; }
};
